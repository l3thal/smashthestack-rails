...
set :scm, :git
set :repository, "git@github.com:l3thal/smashthestack.git"
set :branch, "master"
set :application, "smashthestack"
...
namespace :deploy do
  ...
  task :nginx do
    template = ERB.new(File.read('config/smashthestack.conf.erb'), nil, '<>').result(binding)
